...
// 🧠 FUNÇÃO PARA GARBAGE COLLECTION FORÇADO - RAILWAY COMPATIBLE (SEGURA)
function forceGarbageCollection() {
  try {
    ...
    // Método 2: Limpeza manual segura (sem require('v8') que pode crashar)
    console.log("🧹 GC não disponível - usando limpeza manual segura")

    const before = process.memoryUsage().heapUsed

    // Criar e destruir objetos para sugerir coleta (mais seguro)
    let temp = []
    for (let i = 0; i < 50; i++) {
...
// 🧠 FUNÇÃO DE LIMPEZA SEGURA DE MEMÓRIA (PARA DOWNLOADS) - SEM CRASH
function safeMemoryCleanup() {
  console.log("🧹 === LIMPEZA SEGURA DE MEMÓRIA ===")
  ...
  try {
    ...
    // 2. Limpar caches do Node.js (com cuidado - NÃO REMOVER MÓDULOS ESSENCIAIS)
    // REMOVIDO: A limpeza de require.cache pode causar crashes

    // 3. Criar pressão de memória de forma segura
    let tempArrays = []
    for (let i = 0; i < 20; i++) {
    ...
    console.log(`🎯 Limpeza segura completa: ${totalFreed}MB liberados`)
    console.log(
      `📊 Antes: ${Math.round(before.heapUsed / 1024 / 1024)}MB → Depois: ${Math.round(after.heapUsed / 1024 / 1024)}MB`,
    )
  } catch (error) {
    console.log("❌ Erro na limpeza segura:", error.message)
  }
}

// Alias para compatibilidade com código existente
const ultraAggressiveMemoryCleanup = safeMemoryCleanup
const aggressiveMemoryCleanup = safeMemoryCleanup
...
function checkOptimizationVariables() {
  ...
  // Verificações para GC
  const gcMethods = []

  if (typeof global.gc === "function") {
    gcMethods.push("✅ global.gc() disponível")
  } else {
    gcMethods.push("❌ global.gc() não disponível - usando limpeza manual")
  }
  ...
  if (typeof global.gc === "undefined") {
    console.log("💡 WORKAROUND: Usando métodos seguros de limpeza de memória")
  } else {
...
app.get("/memory", (req, res) => {
  ...
  if (typeof global.gc === "function") {
    ...
    gcMethods.push("Usando limpeza manual segura")
  }
  ...
  res.json({
    ...
    railway_workaround: typeof global.gc !== "function" ? "Usando limpeza manual segura" : "GC nativo funcionando",
  })
})
...
app.get("/test-cookies", async (req, res) => {
  ...
  res.json({
    ...
    summary: {
      ...
      memory_optimization_applied: "🧠 Sistema de limpeza segura de memória ativado (Railway compatible)",
...
app.get("/health", (req, res) => {
  ...
  const stats = {
    ...
    version:
      "6.2.2 - RAILWAY SAFE MEMORY CLEANUP + YOUTUBE BYPASS STRATEGIES + COOKIE VALIDATION FIXED + TWITTER SUPPORT + ECONOMY MODE ENABLED",
    timestamp: new Date().toISOString(),
    limits: {
    ...
    memory_optimization: {
      ...
      railway_workaround: typeof global.gc === "undefined" ? "Safe manual cleanup active" : "Native GC active",
    ...
    security_features: [
      ...
      "🎯 Auto yt-dlp updates",
      "🧠 Railway-compatible safe memory management",
      "🧠 Safe cleanup methods (no crash)",
    ],
    cookies_loaded: {
      google: googleCookiePool.length,
      instagram: instagramCookiePool.length,
...
app.get("/", (req, res) => {
  res.json({
    message:
      "🛡️ WaifuConvert Backend - RAILWAY SAFE MEMORY + YOUTUBE FIX + COOKIE VALIDATION FIXED + TWITTER SUPPORT + ECONOMY MODE ENABLED!",
    version: "6.2.2",
    status: "online - security active + railway safe memory + youtube fix + cookie fix + economy mode enabled",
    security_level: "HIGH",
    limits: {
    ...
    memory_features: [
      "🧠 Railway-compatible safe garbage collection",
      "🧠 No require('v8') - avoids crashes",
      "🧠 Memory usage monitoring",
      "🧠 Automatic cleanup triggered by high memory",
      "🧠 Safe cleanup before downloads",
    ...
    fixes_applied: [
      ...
      "🧠 SAFE Railway memory optimization (no v8 crashes)",
    ...
    features: [
      ...
      "🧠 Railway-safe memory management",
      "🚨 Robust CORS configuration",
    ...
    railway_optimizations: [
      "🚀 Safe memory cleanup (no v8 require)",
      "🚀 No aggressive cache clearing",
      "🚀 Stable cleanup intervals",
...
app.listen(PORT, async () => {
  ...
  console.log("  ✅ Limite de duração: 2 horas (padrão)")
  ...
  console.log("  🧠 RAILWAY SAFE MEMORY: Sistema de limpeza segura (sem v8)")
  ...
  console.log(`  📹 Duração máxima: 2 horas`)
  ...
  console.log("🧠 RECURSOS DE MEMÓRIA RAILWAY (SAFE):")
  console.log("  🧠 Limpeza automática a cada 5 minutos")
  console.log("  🧠 Sem require('v8') - evita crashes")
  console.log("  🧠 Limpeza segura sem remover módulos")
  ...
  console.log("  🧠 Sistema SEGURO de otimização de memória Railway")
  console.log("  🧠 Removido require('v8') que causava crashes")
  ...
  console.log("🧪 Testando limpeza segura na inicialização...")
  safeMemoryCleanup()
